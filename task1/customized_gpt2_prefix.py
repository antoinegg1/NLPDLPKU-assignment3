    ...
    def forward(
        ...
        **kwargs,
        ...
        if input_text is not None:
            for cached_prefix, (cached_key, cached_value) in global_kv_cache.items():
                if input_text.startswith(cached_prefix):  # 如果输入以缓存的前缀开头
                    # 命中缓存，直接返回 KV 对
        ...
        if input_text is not None and use_cache:
            global_kv_cache[input_text] = (key, value)
